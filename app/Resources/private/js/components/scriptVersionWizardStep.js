 ...
 *
 *
 * @class ScriptVersionWizardStep
 */
class ScriptVersionWizardStep {
  /**
   * Creates an instance of Register.
   * @memberof ScriptVersionWizardStep
   */
  constructor() {
  ...
  setAllTabIndex() {
    const $disableElements = document.getElementById('js-script-version-wizardstep').querySelectorAll('input, textarea, select, button, a');
  ...
  _initCarousel() {
    ...
    const carousel = new Siema({
      selector: '#js-script-version-wizardstep',
      duration: 600,
      easing: 'ease-out',
      perPage: 1,
      startIndex: 0,
...
if ($scriptVersionWizardstep) {
  ...
  if (scriptVersionForm) {
    new Abide($(scriptVersionForm));

    scriptVersionForm.addEventListener('change', () => {
      step.autoHeight();
    });

    scriptVersionForm.addEventListener('submit', () => {
      $('#scriptversion_submit').addClass('loader-visible');
  ...
  // ScriptVersionWizardStep navigation
